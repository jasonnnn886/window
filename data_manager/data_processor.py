...
def import_products(file_path):
    ...
    for _, row in df.iterrows():
        # 使用filter().first()替代get()来避免重复产品的问题
...
def process_data(input_file, output_file=None, sheet_to_import=None):
    ...
    try:
        ...
        if input_file:
            ...
            if missing_sheets:
                return False, f"Excel文件缺少以下工作表：{', '.join(missing_sheets)}"
            
            # 按顺序导入数据
            if 'products' in required_sheets:
                try:
                    import_products(input_file)
                    messages.append("产品数据导入成功")
                except Exception as e:
                    return False, f"产品数据导入失败: {str(e)}"
            if 'customers' in required_sheets:
                try:
                    import_customers(input_file)
                    messages.append("客户数据导入成功")
                except Exception as e:
                    return False, f"客户数据导入失败: {str(e)}"
            if 'orders' in required_sheets:
                try:
                    import_orders(input_file)
                    messages.append("订单数据导入成功")
                except Exception as e:
                    return False, f"订单数据导入失败: {str(e)}"
        
        # 如果指定了输出文件，则导出数据
        if output_file:
            try:
                export_data(output_file)
                messages.append("数据导出成功")
            except Exception as e:
                return False, f"数据导出失败: {str(e)}"
